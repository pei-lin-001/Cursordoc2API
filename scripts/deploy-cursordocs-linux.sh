...
COMMAND="${1:-deploy}"
APP_NAME="cursordocs-openai-compatible"
APP_DIR="${CURSORDOCS_DEPLOY_APP_DIR:-${ROOT_DIR}/.cursordocs-linux}"
RUNTIME_DIR="${APP_DIR}/runtime"
LOG_DIR="${APP_DIR}/logs"
RUN_DIR="${APP_DIR}/run"
CACHE_DIR="${APP_DIR}/cache"
ENV_FILE="${APP_DIR}/cursordocs-openai.env"
...
DEFAULT_PORT="${CURSORDOCS_OPENAI_PORT:-8790}"
DEFAULT_HOST="${CURSORDOCS_OPENAI_HOST:-0.0.0.0}"
NODE_MAJOR="${CURSORDOCS_DEPLOY_NODE_MAJOR:-22}"
START_TIMEOUT_SECONDS="${CURSORDOCS_DEPLOY_START_TIMEOUT_SECONDS:-60}"
FORCE_LOCAL_NODE="${CURSORDOCS_DEPLOY_FORCE_LOCAL_NODE:-0}"
ALLOW_NON_LINUX="${CURSORDOCS_DEPLOY_ALLOW_NON_LINUX:-0}"
...
log() {
  printf '[deploy-cursordocs-linux] %s\n' "$*"
}

fail() {
  printf '[deploy-cursordocs-linux] ERROR: %s\n' "$*" >&2
  exit 1
}
...
require_linux() {
  ...
  if [[ "${ALLOW_NON_LINUX}" == "1" ]]; then
    log "检测到非 Linux 平台 ${platform}，因 CURSORDOCS_DEPLOY_ALLOW_NON_LINUX=1 继续执行（仅用于本地联调）"
    return
  fi
...
health_response_is_expected() {
  ...
  [[ "${response_body}" == *'"service":"cursordocs-openai-compatible"'* ]] || return 1
...
ensure_port_available() {
  ...
  if command -v lsof >/dev/null 2>&1; then
    ...
    if [[ -n "${output}" ]]; then
      fail "端口 ${BIND_PORT} 已被其他进程占用，请更换 CURSORDOCS_OPENAI_PORT 或先释放端口"
    fi
    return
  fi
...
ensure_env_file() {
  ...
  cat >"${ENV_FILE}" <<ENVEOF
# Cursor Docs OpenAI Compatible Linux 部署配置
# 首次部署会自动生成此文件，后续可按需修改，然后执行：
#   bash scripts/deploy-cursordocs-linux.sh restart

CURSORDOCS_OPENAI_HOST=${DEFAULT_HOST}
CURSORDOCS_OPENAI_PORT=${DEFAULT_PORT}
CURSORDOCS_OPENAI_API_KEY=
CURSORDOCS_DEFAULT_MODEL=anthropic/claude-sonnet-4.6
CURSORDOCS_MODELS_DISCOVERY_PATH=/cn/docs
CURSORDOCS_REQUEST_TIMEOUT_MS=180000
CURSORDOCS_MODELS_CACHE_TTL_MS=300000
CURSORDOCS_FETCH_DYNAMIC_MODELS=true
CURSORDOCS_ENABLE_ASSISTANT_STEERING=true
CURSORDOCS_SANITIZE_OVERRIDE_META=true
CURSORDOCS_INCLUDE_DEFAULT_CONTEXT=true
CURSORDOCS_DEFAULT_CONTEXT_PATHS=/docs/
CURSORDOCS_EXPOSED_MODELS=Sonnet 4.6=anthropic/claude-sonnet-4.6,GPT-5.1 Codex Mini=openai/gpt-5.1-codex-mini,Gemini 3 Flash=google/gemini-3-flash
CURSORDOCS_MIRROR_REASONING_TO_CONTENT=false
CURSORDOCS_USER_STEERING_TEXT=
CURSORDOCS_ASSISTANT_STEERING_TEXT=
ENVEOF
...
load_env_file() {
  ...
  BIND_HOST="${CURSORDOCS_OPENAI_HOST:-${DEFAULT_HOST}}"
  BIND_PORT="${CURSORDOCS_OPENAI_PORT:-${DEFAULT_PORT}}"
...
source "${ENV_FILE}"
set +a
export CURSORDOCS_BUILD_VERSION="${BUILD_VERSION}"
exec "${NODE_BIN}" "${PNPM_CLI}" exec tsx scripts/cursordocs-openai-compatible.ts
LAUNCHEOF
  chmod +x "${LAUNCH_FILE}"
}
...
logs_service() {
  touch "${LOG_FILE}"
  tail -n "${CURSORDOCS_DEPLOY_LOG_LINES:-80}" -f "${LOG_FILE}"
}

print_usage() {
  cat <<USAGE
用法：bash scripts/deploy-cursordocs-linux.sh [deploy|start|stop|restart|status|logs|install]
